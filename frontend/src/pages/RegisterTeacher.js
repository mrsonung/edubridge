...
const RegisterTeacher = () => {
  const [form, setForm] = useState({ name: '', email: '', password: '', grades: '', subjects: '', qualification: '' });
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      toast.error(err.response?.data?.error || 'Failed to register teacher.');
    }
  };
  ...
  return (
    <>
      <form onSubmit={handleSubmit}>
        ...
        <input name="grades" placeholder="Grades (comma separated)" value={form.grades} onChange={handleChange} required />
